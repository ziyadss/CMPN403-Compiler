...
char *_block(struct AST_Node *block, int label);
...
char *_inv_cond_operation(struct AST_Node *operation, char **jmp)
{
    ...
    *jmp = NULL;
    ...
    case ADD_ASSIGN_OP:
        ...
        fprintf(output_file, "ADD %s, %s, %s\n", name, name, _node(operation->right, 1, 1, 0));
        break;
    case AND_ASSIGN_OP:
        name = operation->left->identifier->name;
    ...
    case ASSIGN_OP:
        if (operation->right->tag == NODE_TYPE_OPERATION)
            ...
            fprintf(output_file, "MOV %s, %s\n", operation->left->identifier->name, _node(operation->right, 1, 1, 0));
        name = operation->left->identifier->name;
        break;
    case DIV_ASSIGN_OP:
        name = operation->left->identifier->name;
    ...
    case XOR_ASSIGN_OP:
        ...
        break;
    case ADD_OP:
        fprintf(output_file, "ADD %s, %s, %s\n", name, _node(operation->left, 1, 1, 0), _node(operation->right, 0, 1, 0));
        break;
    case AND_OP:
        fprintf(output_file, "CMP %s, 0\n", _node(operation->left, 1, 1, 0));
        lbl1 = _label_count();
        fprintf(output_file, "JE L%d\nCMP %s, 0\nJE L%d\nMOV %s, 1\n", lbl1, _node(operation->right, 1, 1, 0), lbl1, name);
        lbl2 = _label_count();
    ...
    case BIT_AND_OP:
        ...
        break;
    case BIT_NOT_OP:
        fprintf(output_file, "XOR %s, %s, -1\n", name, _node(operation->left, 1, 1, 0));
        break;
    case BIT_OR_OP:
        fprintf(output_file, "OR %s, %s, %s\n", name, _node(operation->left, 1, 1, 0), _node(operation->right, 0, 1, 0));
        break;
    ...
    case DIV_OP:
        ...
        break;
    case EQ_OP:;
        char *left_op = operation->left == NULL ? "temp3" : _node(operation->left, 1, 1, 0);
        fprintf(output_file, "CMP %s, %s\n", left_op, _node(operation->right, 0, 1, 0));
        *jmp = "JNE";
        break;
    case GE_OP:
        fprintf(output_file, "CMP %s, %s\n", _node(operation->left, 1, 1, 0), _node(operation->right, 0, 1, 0));
        *jmp = "JL";
        break;
    case GT_OP:
        fprintf(output_file, "CMP %s, %s\n", _node(operation->left, 1, 1, 0), _node(operation->right, 0, 1, 0));
...
char *_if(struct AST_Node *statement, _Bool ternary, char *dest, int label)
{
    char *dst = dest == NULL ? "temp1" : dest;
    char *jmp = _condition(statement->condition);
    int lbl1 = _label_count();
    fprintf(output_file, "%s L%d\n", jmp, lbl1);
    char *src = _node(statement->then_branch, 1, 0, label);
    if (ternary)
    {
        assert(src != NULL);
        fprintf(output_file, "MOV %s, %s\n", dst, src);
    ...
    if (statement->else_branch != NULL)
    {
        ...
        fprintf(output_file, "JMP L%d\n\nL%d:\n", lbl2, lbl1);
        src = _node(statement->else_branch, 1, 0, label);
        if (ternary)
        {
            assert(src != NULL);
            fprintf(output_file, "MOV %s, %s\n", dst, src);
...
void _switch(struct AST_Node *statement)
{
    ...
    if (statement->left->tag == NODE_TYPE_OPERATION)
        ...
        _if(statement->left, 1, "temp3", lbl);
    else
        fprintf(output_file, "MOV temp3, %s\n", _node(statement->left, 1, 1, 0));
...
char *_node(struct AST_Node *statement, _Bool left, _Bool ternary, int label)
{
    ...
    {
    case NODE_TYPE_IF:
        ret = _if(statement, ternary, NULL, label);
        break;
    ...
    case NODE_TYPE_DO_WHILE:
        ...
        break;
    case NODE_TYPE_FOR:
        _node(statement->initialization, 1, 0, label);
        _while(statement);
        break;
...
char *_block(struct AST_Node *block, int label)
{
    for (unsigned int i = 0; i < block->statements_count - 1; i++)
        _node(block->statements[i], 1, 0, label);

    return _node(block->statements[block->statements_count - 1], 1, 0, label);
}
...
void _operation_dst(char *identifier, struct AST_Node *operation)
{
    ...
    case CALL_OP:
        ...
        break;
    case ADD_ASSIGN_OP:
        fprintf(output_file, "ADD %s, %s, %s\n", identifier, identifier, _node(operation->right, 1, 1, 0));
        break;
    case AND_ASSIGN_OP:
        fprintf(output_file, "AND %s, %s, %s\n", identifier, identifier, _node(operation->right, 1, 1, 0));
        break;
    ...
    case ADD_OP:
        if (operation->left == NULL)
            fprintf(output_file, "MOV %s, %s\n", identifier, _node(operation->right, 1, 1, 0));
        else
            fprintf(output_file, "ADD %s, %s, %s\n", identifier, _node(operation->left, 1, 1, 0), _node(operation->right, 0, 1, 0));
        break;
    case AND_OP:
        fprintf(output_file, "CMP %s, 0\n", _node(operation->left, 1, 1, 0));
        lbl1 = _label_count();
        fprintf(output_file, "JE L%d\nCMP %s, 0\nJE L%d\nMOV %s, 1\n", lbl1, _node(operation->right, 1, 1, 0), lbl1, identifier);
        lbl2 = _label_count();
...
char *_operation(struct AST_Node *operation, _Bool left)
{
    ...
    case RET_OP:
        ...
        if (operation->left != NULL)
        {
            if (operation->left->tag == NODE_TYPE_OPERATION)
                ...
                fprintf(output_file, "MOV %s, %s\n", ret, _node(operation->left, 1, 1, 0));
        }
    ...
    case ADD_ASSIGN_OP:
        ...
        fprintf(output_file, "ADD %s, %s, %s\n", ret, ret, _node(operation->right, 1, 1, 0));
        break;
    case AND_ASSIGN_OP:
        ret = operation->left->identifier->name;
        fprintf(output_file, "AND %s, %s, %s\n", ret, ret, _node(operation->right, 1, 1, 0));
    ...
    case XOR_ASSIGN_OP:
        ...
        break;
    case ADD_OP:
        fprintf(output_file, "ADD %s, %s, %s\n", ret, _node(operation->left, 1, 1, 0), _node(operation->right, 0, 1, 0));
        break;
    case AND_OP:
        fprintf(output_file, "CMP %s, 0\n", _node(operation->left, 1, 1, 0));
        lbl1 = _label_count();
        fprintf(output_file, "JE L%d\nCMP %s, 0\nJE L%d\nMOV %s, 1\n", lbl1, _node(operation->right, 1, 1, 0), lbl1, ret);
        lbl2 = _label_count();
        fprintf(output_file, "JMP L%d\n\nL%d:\nMOV %s, 0\n\nL%d:\n", lbl2, lbl1, ret, lbl2);
        break;
    case BIT_AND_OP:
